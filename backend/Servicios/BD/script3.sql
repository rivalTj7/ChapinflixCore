...
-- ---------------------------------------------------------
-- 0) Utilitaria: mapea user_id INTEGER -> UUID sintético
--    (mismo patrón que ya usaste en carruseles)
--    Si más adelante cambias view_history.user_id a INTEGER,
--    podrás eliminar esta función y el casteo en los INSERTs.
-- ---------------------------------------------------------
CREATE OR REPLACE FUNCTION fn_user_int_to_uuid(p_user_id INTEGER)
RETURNS UUID
LANGUAGE sql
AS $$
  SELECT ('00000000-0000-0000-0000-' || LPAD(p_user_id::text, 12, '0'))::uuid;
$$;

-- ---------------------------------------------------------
-- 1) ¿Puede ver esta película?
--    No lee usuarios: usa solamente p_is_paid (del JWT).
--    Revisa is_active, ventana de disponibilidad y si es free.
-- ---------------------------------------------------------
CREATE OR REPLACE FUNCTION fn_movie_can_view(
  p_movie_id UUID,
  p_is_paid  BOOLEAN,
...
DECLARE
  ...
  v_until          TIMESTAMPTZ;
BEGIN
  SELECT m.is_active, m.is_free, m.available_from, m.available_until
    INTO v_is_active, v_is_free, v_from, v_until
  FROM movie m
  WHERE m.id = p_movie_id;

  IF NOT FOUND THEN
    RETURN QUERY SELECT FALSE, 'movie:not_found';
    RETURN;
  END IF;

  IF v_is_active IS NOT TRUE THEN
    RETURN QUERY SELECT FALSE, 'movie:inactive';
    RETURN;
  END IF;

  IF v_from IS NOT NULL AND v_from > p_now THEN
    RETURN QUERY SELECT FALSE, 'movie:not_yet_available';
    RETURN;
  END IF;

  IF v_until IS NOT NULL AND v_until < p_now THEN
    RETURN QUERY SELECT FALSE, 'movie:expired';
    RETURN;
  END IF;

  -- Regla de acceso: gratis o usuario pagado
  IF v_is_free OR p_is_paid THEN
    RETURN QUERY SELECT TRUE, NULL::TEXT;
  ELSE
    RETURN QUERY SELECT FALSE, 'payment:required';
  END IF;
END$$;

-- ---------------------------------------------------------
-- 2) Detalle enriquecido por slug (para la pantalla "Ver")
--    Devuelve metadatos + arrays de categorías e imágenes.
-- ---------------------------------------------------------
CREATE OR REPLACE FUNCTION fn_movie_detail_by_slug(
  p_slug TEXT
)
RETURNS TABLE (
  movie_id          UUID,
...
AS $$
  WITH base AS (
    SELECT
      ...
      m.available_until
    FROM movie m
    LEFT JOIN classification cls ON cls.id = m.classification_id
    LEFT JOIN studio         std ON std.id = m.studio_id
    WHERE m.slug = lower(p_slug)
  ),
  cats AS (
    SELECT mc.movie_id, array_agg(c.slug ORDER BY c.slug) AS cat_slugs
    FROM movie_category mc
    JOIN category c ON c.id = mc.category_id
    WHERE mc.movie_id = (SELECT id FROM base)
  ...
  imgs AS (
    ...
    FROM movie_image mi
    WHERE mi.movie_id = (SELECT id FROM base)
    GROUP BY mi.movie_id
  )
  SELECT
    b.id, b.title, b.slug,
    b.synopsis_short, b.synopsis_long,
    b.duration_minutes, b.release_date,
    b.classification, b.studio_name,
    b.language, b.country,
    b.poster_url, b.banner_url, b.trailer_url,
    b.is_free, b.available_from, b.available_until,
    COALESCE(c.cat_slugs, ARRAY[]::TEXT[]) AS categories,
    COALESCE(i.gallery, '[]'::jsonb)        AS images
  FROM base b
  LEFT JOIN cats c ON c.movie_id = b.id
  LEFT JOIN imgs i ON i.movie_id = b.id;
$$;

-- ---------------------------------------------------------
-- 3) Play: valida acceso y registra la visualización.
--    - p_is_paid viene del JWT (no se consulta users).
--    - p_user_id puede ser NULL (ej. sesión anónima); si viene,
--      se registra en view_history (y el trigger suma view_count).
--    Devuelve allowed + reason + un payload mínimo para el frontend.
-- ---------------------------------------------------------
CREATE OR REPLACE FUNCTION fn_movie_play(
  p_user_id  INTEGER,
  p_slug     TEXT,
  p_is_paid  BOOLEAN,
  p_now      TIMESTAMPTZ DEFAULT now()
...
DECLARE
  v_movie_id UUID;
  v_title TEXT; v_slug TEXT; v_synopsis_short TEXT;
  v_poster_url TEXT; v_banner_url TEXT; v_trailer_url TEXT;
  v_is_free BOOLEAN;
  v_allowed  BOOLEAN;
  v_reason   TEXT;
BEGIN
  SELECT m.id, m.title, m.slug, m.synopsis_short, m.poster_url, m.banner_url, m.trailer_url, m.is_free
    INTO v_movie_id, v_title, v_slug, v_synopsis_short, v_poster_url, v_banner_url, v_trailer_url, v_is_free
  FROM movie m
  WHERE m.slug = lower(p_slug);

  IF NOT FOUND THEN
    allowed := FALSE; denial_reason := 'movie:not_found';
    movie_id := NULL; title := NULL; slug := NULL;
    synopsis_short := NULL; poster_url := NULL; banner_url := NULL; trailer_url := NULL; is_free := NULL;
    RETURN NEXT; RETURN;
  END IF;

  -- 👇 cambio clave: alias "mv" elimina ambigüedad
  SELECT mv.allowed, mv.denial_reason
    INTO v_allowed, v_reason
  FROM fn_movie_can_view(v_movie_id, p_is_paid, p_now) AS mv;

  IF v_allowed IS NOT TRUE THEN
    allowed := FALSE; denial_reason := v_reason;
    movie_id := v_movie_id;
    title := v_title; slug := v_slug; synopsis_short := v_synopsis_short;
    poster_url := v_poster_url; banner_url := v_banner_url; trailer_url := v_trailer_url; is_free := v_is_free;
    RETURN NEXT; RETURN;
  END IF;

  IF p_user_id IS NOT NULL THEN
    INSERT INTO view_history(user_id, movie_id, viewed_at)
    VALUES (fn_user_int_to_uuid(p_user_id), v_movie_id, p_now);
  END IF;

  allowed := TRUE; denial_reason := NULL;
  movie_id := v_movie_id;
  title := v_title; slug := v_slug; synopsis_short := v_synopsis_short;
  poster_url := v_poster_url; banner_url := v_banner_url; trailer_url := v_trailer_url; is_free := v_is_free;
  RETURN NEXT;
...
-- Parche en fn_movie_play_by_id: alias a la subconsulta
CREATE OR REPLACE FUNCTION fn_movie_play_by_id(
  p_user_id  INTEGER,
  p_movie_id UUID,
  p_is_paid  BOOLEAN,
  p_now      TIMESTAMPTZ DEFAULT now()
...
DECLARE
  ...
  v_reason   TEXT;
BEGIN
  SELECT m.title, m.slug, m.synopsis_short, m.poster_url, m.banner_url, m.trailer_url, m.is_free
    INTO v_title, v_slug, v_synopsis_short, v_poster_url, v_banner_url, v_trailer_url, v_is_free
  FROM movie m
  WHERE m.id = p_movie_id;
  ...
  -- 👇 cambio clave: alias "mv"
  SELECT mv.allowed, mv.denial_reason
    INTO v_allowed, v_reason
  FROM fn_movie_can_view(p_movie_id, p_is_paid, p_now) AS mv;
  ...
  IF p_user_id IS NOT NULL THEN
    INSERT INTO view_history(user_id, movie_id, viewed_at)
    VALUES (fn_user_int_to_uuid(p_user_id), p_movie_id, p_now);
  END IF;
