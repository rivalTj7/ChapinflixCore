-- =========================================================
-- Custom User Management System for Supabase
-- PERSONAL USE ONLY - NOT FOR PRODUCTION
-- Uses 'app_users' to avoid conflicts with Supabase auth.users
-- =========================================================

-- -----------------------------
-- Create custom schema (optional but recommended)
-- -----------------------------
CREATE SCHEMA IF NOT EXISTS app;

-- -----------------------------
-- Tables (using app schema to avoid conflicts)
-- -----------------------------

-- Custom Users table (renamed to avoid conflict with auth.users)
CREATE TABLE IF NOT EXISTS app.app_users (
    id BIGSERIAL PRIMARY KEY,  -- Changed to BIGSERIAL for Supabase compatibility
    ...
    locked_until TIMESTAMPTZ,  -- Changed to TIMESTAMPTZ for Supabase
    -- commercial/roles kept here
    ...
    updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()   -- Changed to TIMESTAMPTZ
);
...
CREATE INDEX IF NOT EXISTS idx_email_verification_tokens_token
  ON app.email_verification_tokens(token);
...
CREATE POLICY "Enable all for app_users" ON app.app_users
    FOR ALL USING (true) WITH CHECK (true);

CREATE POLICY "Enable all for email_verification_tokens" ON app.email_verification_tokens
    FOR ALL USING (true) WITH CHECK (true);

CREATE POLICY "Enable all for refresh_tokens" ON app.refresh_tokens
    FOR ALL USING (true) WITH CHECK (true);
...
LANGUAGE plpgsql
SECURITY DEFINER  -- Added for Supabase function security
AS $$
DECLARE
  v_sql TEXT;
...
CREATE TRIGGER update_app_users_updated_at BEFORE UPDATE
    ON app.app_users FOR EACH ROW EXECUTE FUNCTION app.update_updated_at_column();