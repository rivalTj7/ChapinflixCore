...
export default function SubscriptionPage() {
  ...
  const fetchPrices = async () => {
    try {
      const response = await fetch("http://34.135.146.173.nip.io/pago/api/prices");
  ...
  const checkSubscriptionStatus = async () => {
    ...
    try {
      const response = await fetch("http://34.135.146.173.nip.io/pago/api/me/subscription", {
        headers: {
  ...
  const handleSubscribe = async (priceId?: string) => {
    ...
    try {
      const response = await fetch("http://34.135.146.173.nip.io/pago/api/checkout/session", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          ...(token ? { Authorization: `Bearer ${token}` } : {}),
  ...
  const handleCancelSubscription = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch("http://34.135.146.173.nip.io/pago/api/subscriptions/cancel", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          ...(token ? { Authorization: `Bearer ${token}` } : {}),
