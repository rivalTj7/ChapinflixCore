...
export default function Catalog() {
    ...
    try {
      ...
      if (authResponse.ok) {
        ...
        try {
          ...
          const userDetailResponse = await fetch(`https://34.10.139.168.nip.io/ususarios/users/${authProfile.id}/detail`, {
            headers: {
              "Authorization": `Bearer ${token}`,
              "Content-Type": "application/json"
            }
  ...
  const handleEditProfile = async () => {
    ...
    try {
      ...
      const response = await fetch("https://34.10.139.168.nip.io/auth/api/auth/profile", {
        method: "PUT",
        headers: {
          "Authorization": `Bearer ${token}`,
          "Content-Type": "application/json"
  ...
  const loadCategories = async () => {
    try {
      const res = await fetch("https://34.10.139.168.nip.io/vercatalogo/catalog/categories");
  ...
  const loadMovies = async (type: string) => {
    setLoading(true);
    setActiveTab(type);
    let url = "https://34.10.139.168.nip.io/vercatalogo/catalog/";
  ...
  const loadByCategory = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`https://34.10.139.168.nip.io/vercatalogo/catalog/category/${selectedCategory}?limit=20&offset=0`);
